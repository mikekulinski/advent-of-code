...
// orderHands orders the hands from weakest to strongest.
...
func getHandRank(hand Hand) Rank {
...
func getCardCounts(hand Hand) map[string]int {
...
func compareCards(a, b Hand) bool {
	for i := 0; i < len(a.Cards); i++ {
		...
		bCard := b.Cards[i]
		if aCard != bCard {
			return suitRank[aCard] > suitRank[bCard]
		}
	}
	return false
}
...
var (
	suitRankWithJokers = map[string]int{
		"A": 0,
		"K": 1,
		"Q": 2,
		"T": 3,
...
// orderHandsWithJokers orders the hands from weakest to strongest.
...
func compareHandsWithJokers(a, b Hand) bool {
	aType := getHandRankWithJokers(a)
	bType := getHandRankWithJokers(b)
	if aType != bType {
		return aType > bType
	}
	return compareCardsWithJokers(a, b)
}

func getHandRankWithJokers(hand Hand) Rank {
	cardCounts := getCardCountsWithJokers(hand)
	if len(cardCounts) == 1 {
		return FiveOfAKind
	} else if len(cardCounts) == 2 {
		for _, count := range cardCounts {
...
func compareCardsWithJokers(a, b Hand) bool {
	for i := 0; i < len(a.Cards); i++ {
		aCard := a.Cards[i]
		bCard := b.Cards[i]
		if aCard != bCard {
			return suitRankWithJokers[aCard] > suitRankWithJokers[bCard]
		}
	}
	return false
}

func getCardCountsWithJokers(hand Hand) map[string]int {
	cardCounts := map[string]int{}
	for _, card := range hand.Cards {
		cardCounts[card]++
	}

	// Now turn jokers into whatever card is best for the hand.
	if cardCounts["J"] > 0 {
		highestCount := 0
		highestCard := ""
		...
		// If there are no other cards, then we have all jokers, so don't substitute anything.
		if highestCard != "" {
			cardCounts[highestCard] += cardCounts["J"]
			delete(cardCounts, "J")
		}
	}
	return cardCounts
