...
func Part1(lines []string) (int, error) {
	...
	total := totalExtrapolatedValues(sequences, nextExtrapolatedValue)
	return total, nil
}
...
func totalExtrapolatedValues(sequences [][]int, extrapolateValue func([]int) int) int {
	total := 0
	for _, seq := range sequences {
		total += extrapolateValue(seq)
	}
	return total
}

func nextExtrapolatedValue(seq []int) int {
	tree := buildTree(seq)
	extrapolatedValue := 0
	for i := len(tree) - 1; i >= 0; i-- {
		line := tree[i]
		extrapolatedValue += line[len(line)-1]
	}
	return extrapolatedValue
}

func buildTree(seq []int) [][]int {
	tree := [][]int{seq}
	// Continue until the last line in the tree is all zeros.
	for currentLine := tree[len(tree)-1]; !allZeros(currentLine); currentLine = tree[len(tree)-1] {
		var nextLine []int
		for i := 0; i < len(currentLine)-1; i++ {
			nextLine = append(nextLine, currentLine[i+1]-currentLine[i])
		}
		tree = append(tree, nextLine)
	}
	return tree
}
...
func Part2(lines []string) (int, error) {
	sequences := parseInput(lines)
	total := totalExtrapolatedValues(sequences, firstExtrapolatedValue)
	return total, nil
}

func firstExtrapolatedValue(seq []int) int {
	tree := buildTree(seq)
	extrapolatedValue := 0
	for i := len(tree) - 1; i >= 0; i-- {
		line := tree[i]
		extrapolatedValue = line[0] - extrapolatedValue
	}
	return extrapolatedValue
}
