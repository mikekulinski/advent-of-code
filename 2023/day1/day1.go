...
sending you ("the sky") and why your map looks mostly blank ("you sure ask a lot of questions")
...
func Part1(input []string) (int, error) {
	...
	for _, line := range input {
		numStr := parseInts(line)
		// Only use the first and last digits.
		num, err := strconv.Atoi(numStr[0] + numStr[len(numStr)-1])
		if err != nil {
			return 0, err
		}
...
func parseInts(line string) []string {
	numStr := []string{}
...
func Part2(input []string) (int, error) {
	...
	for _, line := range input {
		...
		// Only use the first and last digits.
		first, last := string(nums[0]), string(nums[len(nums)-1])
...
func parseNumbers(line string) string {
	...
	for i := 0; i < len(line); i++ {
		for j := i + 1; j <= len(line); j++ {
			window := line[i:j]
			// If the window already is a number, just add it.
			if _, err := strconv.Atoi(window); err == nil {
				newLine += window
				break
			}
			// If the window is a number written as text, add that number.
			if integer, ok := numberMap[window]; ok {
				newLine += strconv.Itoa(integer)
				break
			}
		}
	}
