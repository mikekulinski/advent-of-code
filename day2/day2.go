...
cubes and 4 red cubes; the second set is 1 red cube, 2 green cubes, and 6 blue cubes; the third set is only 2 green cubes.
...
func Part1(input []string) (int, error) {
	...
	for _, line := range input {
		...
		maxColors := getMaxColors(game)
		if isValidGame(maxColors) {
			sumOfIDs += game.ID
		}
	}
	return sumOfIDs, nil
...
func parseLine(line string) (*Game, error) {
	...
	// Get the gameID from the first section.
	gameID, err := strconv.Atoi(strings.TrimPrefix(sections[0], "Game "))
	if err != nil {
		return nil, err
	}
	game.ID = gameID

	// Get the reveals from the second section.
	reveals := strings.Split(strings.TrimSpace(sections[1]), ";")
	for _, reveal := range reveals {
		reveal, err := parseReveal(reveal)
		if err != nil {
			return nil, err
		}
...
func parseReveal(r string) (*Reveal, error) {
	r = strings.TrimSpace(r)

	reveal := &Reveal{
		Counts: map[Color]int{},
	}
	colorAmount := strings.Split(r, ",")
	for _, ca := range colorAmount {
		split := strings.Split(strings.TrimSpace(ca), " ")
		amount, err := strconv.Atoi(strings.TrimSpace(split[0]))
		if err != nil {
			return nil, err
		}
		color := Color(strings.TrimSpace(split[1]))
		reveal.Counts[color] = amount
	}
	return reveal, nil
}

func getMaxColors(game *Game) map[Color]int {
	maxColors := map[Color]int{}
	for _, reveal := range game.Reveals {
		for color, count := range reveal.Counts {
...
func isValidGame(maxColors map[Color]int) bool {
	for color, count := range gameCheck {
...
func (g *Game) String() string {
	str := fmt.Sprintf("Game: %d\n", g.ID)
	for _, reveal := range g.Reveals {
		str += fmt.Sprintf("\tReveal: %+v\n", reveal)
	}
	return str
}
...
func Part2(input []string) (int, error) {
	...
	for _, line := range input {
		...
		maxColors := getMaxColors(game)
		sumOfPowers += getPowerSet(maxColors)
	}
	return sumOfPowers, nil
}

func getPowerSet(maxColors map[Color]int) int {
	powerSet := 1
	for _, count := range maxColors {
		powerSet *= count
	}
	return powerSet
}
